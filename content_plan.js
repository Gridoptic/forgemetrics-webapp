    ...
    function rubricsBlock(inWeek, pubDone) {
        ...
        return '<div class="cp-sec">' + secHead('Рубрики канала', note) +
            ...
            (wOn ? '<div class="cp-bfoot"><i class="ti ti-bolt"></i><span>' +
                esc(T('Рубрика задаёт формат, а конкретную тему каждого поста — какую добавку, теорию или протокол разобрать — придумывает сборка под сюжет недели; темы появятся на карточках постов. Число рядом с описанием — сколько постов канала попало в рубрику, просмотры и полоска — их средний охват и доля от лучшей рубрики (замеры от 3 постов). Нажатие включает и выключает рубрику; крестик возвращает её в предложения — он есть только у добавленных тобой.')) +
                '</span></div>' : '') +
            (inWeek && _rubChanged
                ? (pubDone
