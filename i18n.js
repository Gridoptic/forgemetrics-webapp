(function () {
    ...
    var TEMPLATES = [
        { re: /^Обновление стиля · (\d+) Forge$/, k: 'Обновление стиля · %1 Forge' },
        ...
        { re: /^Каналов подключено: (\d+) · по тарифу: (\d+)$/, k: 'Каналов подключено: %1 · по тарифу: %2' },
        ...
        { re: /^Другу −(\d+)% на первый месяц и расширенный триал: (\d+) дней вместо (\d+)\. Кредитами оплачивается до половины счёта за тариф или продвижение\.$/, k: 'Другу −%1% на первый месяц и расширенный триал: %2 дней вместо %3. Кредитами оплачивается до половины счёта за тариф или продвижение.' },
