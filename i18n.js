(function () {
    ...
    var TEMPLATES = [
        { re: /^в ([\d.,]+) раза ниже обычного$/, k: 'в %1 раза ниже обычного' },
        { re: /^по (\d+) рекламному посту$/, k: 'по %1 рекламному посту' },
        { re: /^по (\d+) рекламным постам$/, k: 'по %1 рекламным постам' },
        { re: /^найдено (\d+), нужно от 3$/, k: 'найдено %1, нужно от 3' },
        { re: /^Баланса хватит на (\d+) (.+)$/, k: 'Баланса хватит на %1 %2' },
        { re: /^Начисление автоматически после оплаты\. Приглашённый получает −(\d+)% на первое пополнение и \+(\d+) Forge к стартовому запасу\.$/, k: 'Начисление автоматически после оплаты. Приглашённый получает −%1% на первое пополнение и +%2 Forge к стартовому запасу.' },
        { re: /^Приглашённый регистрируется по ней и получает −(\d+)% на первое пополнение Forge и \+(\d+) Forge к стартовому запасу\.$/, k: 'Приглашённый регистрируется по ней и получает −%1% на первое пополнение Forge и +%2 Forge к стартовому запасу.' },
        { re: /^от (\d+) оплатившего$/, k: 'от %1 оплатившего' },
        { re: /^от (\d+) оплативших$/, k: 'от %1 оплативших' },
        { re: /^Друг купил 900 Forge — тебе (\d+)\. И так с каждой его покупки, без ограничения по числу платежей\. Другу −(\d+)% на первое пополнение и \+(\d+) Forge к стартовому запасу\.$/, k: 'Друг купил 900 Forge — тебе %1. И так с каждой его покупки, без ограничения по числу платежей. Другу −%2% на первое пополнение и +%3 Forge к стартовому запасу.' },
        { re: /^Друг регистрируется по ней: −(\d+)% на первое пополнение Forge и \+(\d+) Forge к стартовому запасу\.$/, k: 'Друг регистрируется по ней: −%1% на первое пополнение Forge и +%2 Forge к стартовому запасу.' },
        { re: /^(\d+)% с каждого пополнения — в Forge$/, k: '%1% с каждого пополнения — в Forge' },
        { re: /^(\d+)% в Forge с каждого пополнения друга$/, k: '%1% в Forge с каждого пополнения друга' },
