...
(function () {
    ...
    var TEMPLATES = [
        ...
        // генерация 2.0 + рефералка модель C + промо (13.07): динамические строки с числами
        ...
        // зачистка по вычитке 13.07 (вечер): бэкенд-сабтайтл меню, триал-баннер, лидерборд, счётчики
    ...
    // Перевод отрезка: точный ключ → шаблон → разделитель по краям («· 60 оплативших»).
    // Рекурсия ограничена: stripSepTranslate снимает разделитель и зовёт _segTr уже без него.
    // edge НЕ используем (рискует задеть данные — названия/описания каналов).
    function _segTr(seg) {
        if (!seg) return null;
        var r = window.t(seg);
        if (r && r !== seg) return r;
