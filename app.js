...
function openLangPicker() {
    ...
    const cur = getLang();
    const rows = I18N.supported.map((l) => `<button class="lang-row${l === cur ? ' on' : ''}" data-l="${l}"><span class="lc">${l.toUpperCase()}</span><span class="fl">${I18N.flags[l]}</span><span class="nm">${escapeHtml(I18N.names[l])}</span>${l === cur ? '<i class="ti ti-check ck"></i>' : ''}</button>`).join('');
