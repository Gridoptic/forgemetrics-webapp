(function () {
    ...
    var TERMS = [
        ...
        ['ROI · окупаемость', 'Главный итоговый показатель: чистая прибыль ÷ затраты на размещение × 100%. Пример: пост за 5 000 ₽ принёс продаж на 12 000 ₽ при себестоимости 4 000 ₽ — прибыль 3 000 ₽, ROI 60%. Выше нуля — размещение в плюсе, ниже — в минусе. Считай ROI по каждому каналу отдельно: усреднённая цифра по кампании прячет каналы-дыры, которые тянут бюджет вниз.'],
        ['CTR · кликабельность', 'Доля увидевших пост, которые кликнули по ссылке: клики ÷ охват поста × 100%. В размещениях в каналах ориентир 0,5–2%: выше — креатив и аудитория попали друг в друга, ниже — пост не цепляет или аудитория не та. CTR — метрика твоего креатива, а не канала: один и тот же канал даст разный CTR разным объявлениям. Меряется только метками в ссылке.'],
        ...
        ['CPC · цена клика', 'Стоимость одного перехода: цена размещения ÷ клики. Мост между ценой показа и ценой результата: CPM говорит, почём внимание, CPC — почём заинтересованный человек. Сравнивай CPC каналов только при одинаковых креативах — иначе сравниваешь не каналы, а объявления.'],
        ...
        ['ROAS · возврат на рекламу', 'Вся выручка с размещения ÷ его стоимость × 100%. Отличие от ROI: ROAS считает выручку, ROI — чистую прибыль. Пример: пост за 5 000 ₽ дал продаж на 15 000 ₽ — ROAS 300%; но при марже 30% прибыль всего 4 500 ₽, и ROI уже отрицательный. Порог безубыточности: ROAS должен превышать 100% ÷ маржа — при марже 30% это 333%.'],
        ['ДРР · доля рекламных расходов', 'Затраты на рекламу ÷ выручка с неё × 100% — зеркальный к ROAS показатель, принятый в e-commerce. Чем ниже, тем здоровее экономика. Простое правило: ДРР выше твоей маржи — продажи идут в убыток, сколько бы заказов ни капало.'],
        ['CPF · цена подписчика', 'Потрачено ÷ новые подписчики с размещения. Главная метрика, когда цель закупки — рост собственного канала. Пример: пост за 5 000 ₽ привёл 250 подписчиков — CPF 20 ₽. Смотри не только цену, но и качество: дешёвый мотивированный трафик отваливается за неделю и глушит охват канала. Прирост замеряй в первые 24–48 часов после выхода поста — дальше вклад размещения уже не отделить.'],
        ['CAC · стоимость клиента', 'Во сколько обходится один новый платящий клиент: все расходы на привлечение ÷ число новых клиентов. Шире CPA: сюда входит не только бюджет размещений, но и креативы, сервисы, руки. Сам по себе ни о чём — оценивается только в паре с LTV.'],
