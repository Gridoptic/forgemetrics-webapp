(function () {
    ...
    function openBuyFilters() {
        ...
        bg.innerHTML = '<div class="fmx-cfm-box fmx-bf-compact" style="left:50%;transform:translateX(-50%);margin-left:0;width:calc(100vw - 20px);max-width:480px;bottom:12px;"><div class="fmx-cfm-t" style="margin-bottom:10px;display:flex;align-items:center;gap:8px;"><i class="ti ti-adjustments-horizontal" style="color:#818cf8;"></i> Фильтры' +
            ...
            _bfPair('Reach Rate, %', 'fmx-bf-er', _fErMin, 'fmx-bf-erx', _fErMax, true) +
    ...
    var _AL_RANGES = [['p', 'Цена не дороже, ₽', 'mx'], ['cpm', 'CPM не дороже, ₽', 'mx'], ['s', 'Подписчиков от', 'mn'],
        ['r', 'Охват от', 'mn'], ['err', 'Reach Rate от, %', 'mn'], ['er', 'ERR от, %', 'mn'],
    ...
    var CMP_ROWS = [
        ...
        { k: 'er', label: 'Reach Rate', dir: 'up', get: function (l) { return l.er != null ? l.er : null; }, fmt: function (v) { return (Math.round(v * 10) / 10) + '%'; } },
    ...
    function _fmrBlocksBuy(l) {
        ...
        if (rr != null && rstat) {
            ...
            rrHtml = '<div class="fmr-line" style="margin-top:5px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">' + rrWarn + 'Reach Rate <b style="color:' + rrCol + ';">' + rr + '%</b> <span style="font-size:11px;color:' + rrCol + ';font-weight:600;">' + _esc(rstat) + '</span>' + normTxt + '<i class="fmr-i ti ti-info-circle push" data-fi="rr"></i></div>' +
                '<div class="fmr-info" data-finfo="rr">Reach Rate = охват ÷ подписчики — какой процент подписчиков видит пост. Норму смотрим по размеру канала (у больших она ниже — это нормально): микро до 5к 25–50%, малый 5–10к 18–35%, средний 10к–100к 7–22%, крупный 100к–1М 6–16%, миллионник 3–10%. Нормы выведены из реальной базы каналов. Слишком низко для своего размера — признак мёртвой базы; в разы выше нормы — повод проверить источник охвата.' + rrAnom + '</div>';
        }
        var erHtml = '';
        if (l.engagement_percent != null) {
            ...
            erHtml = '<div class="fmr-line" style="margin-top:5px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">Вовлечённость (ERR) <b style="color:' + _erCol + ';">' + _ervTxt + '%</b> <span style="font-size:11px;color:' + _erCol + ';font-weight:600;">' + _erStat + '</span>' + erSub + '<i class="fmr-i ti ti-info-circle push" data-fi="er"></i></div>' +
                '<div class="fmr-info" data-finfo="er">ERR (вовлечённость по охвату) = (реакции + репосты + комментарии) ÷ охват — какая доля увидевших пост взаимодействует с ним. Именно ERR, а не ER: ER на рынке считают от числа подписчиков, и он занижен из-за тех, кто пост не видел. Живой сигнал: просмотры накрутить дёшево, взаимодействия — нет. Ориентир: до 1% — низкая, 1–3.5% — норма, выше 3.5% — высокая (у новостных ниже, они живут репостами). Если взаимодействия скрыты — ER не показываем.</div>';
        }
    ...
    function _heroTiles(l, mode) {
        ...
        var tiles =
            ...
            _htile('Reach Rate', rr != null ? rr + '%' : '—', rrCol, rstat || 'уточняется', rstat ? rrCol : '') +
            _htile('ERR', ervTxt, erCol, erStat, erStat ? erCol : '') +
    ...
    function fullCard(l) {
        ...
        return '<div class="fmx-cwrap"><div class="fmx-card' + (glowOn ? ' fmx-prem' : '') + (fullBg ? ' fmx-fullbg' : '') + '" data-u="' + _esc(l.username) + '">' + cbgHtml + stkHtml + covBdg +
            ...
            (l.health_score != null ? '<div class="fmr-info" data-finfo="health">Индекс здоровья канала (0–100): насколько канал живой и качественный как площадка — вовлечённость, Reach Rate, стабильность охватов, нет ли накрутки. Считается из тех же метрик, что видны выше, поэтому не противоречит им. Зелёный — хорошо, жёлтый — средне, красный — с осторожностью.</div>' : '') +
            bodyBdg +
    ...
    function _pwMetrics(l) {
        ...
        if (l.er != null && l.reach_status) {
            ...
            _xtra += '<div style="font-size:11px;color:#9aa0b8;margin-top:6px;">Reach Rate — <b style="color:' + _rc + ';">' + _esc(l.reach_status) + '</b>' + _nrm + '</div>';
        }
        if (l.engagement_percent != null) {
            var _eb = [];
