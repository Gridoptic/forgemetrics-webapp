(function () {
    ...
    var TERMS = [
        ...
        ['Накрутка', 'Искусственные подписчики, просмотры или реакции. Признаки: большая база при крошечном охвате (низкий RR), резкие скачки просмотров без причины, ровные «полки» и ночные пики на графике, ER около нуля. Как проверить вручную: открой комментарии — живые ли профили с историей и осмысленные ли тексты; сопоставь реакции с комментариями — много реакций при пустом обсуждении часто означает купленные реакции (они дешевле просмотров и встречаются чаще, чем думают). Индикатор здоровья подсвечивает подозрительные каналы жёлтым или красным, а бейдж «Без накрутки» выдаётся только при достаточных данных и без следов накрутки.'],
    ...
    var TIPS = [
        ...
        ['Владельцу · держи метрики честными', 'Накрутка подписчиков роняет RR и включает красный светофор — опытный закупщик это видит и уходит. Живой канал с бейджем «Без накрутки» вызывает больше доверия, чем формально крупный с выгоревшей базой. Честные метрики — это актив, который работает на цену размещения.'],
    ...
    function openBuyFilters() {
        ...
        bg.innerHTML = '<div class="fmx-cfm-box fmx-bf-compact" style="left:50%;transform:translateX(-50%);margin-left:0;width:calc(100vw - 20px);max-width:480px;bottom:12px;"><div class="fmx-cfm-t" style="margin-bottom:10px;display:flex;align-items:center;gap:8px;"><i class="ti ti-adjustments-horizontal" style="color:#818cf8;"></i> Фильтры' +
            ...
            _bfPair('RR, %', 'fmx-bf-er', _fErMin, 'fmx-bf-erx', _fErMax, true) +
    ...
    function paintMine() {
        ...
        /* точечные скидки: открываем календарь владельца сразу в режиме «Скидки» */
    ...
    var _RF_RANGES = [['s', 'Подписчики'], ['p', 'Цена поста, ₽'], ['r', 'Охват'], ['err', 'RR, %'], ['er', 'ER, %'], ['cpm', 'CPM, ₽'], ['h', 'Индекс'], ['age', 'Возраст, мес'], ['adp', 'Реклама, %']];
    ...
    /* режим «Скидки» в календаре владельца: точечные горящие даты (свой % на дату/время) */
    ...
    function _buyerSlotsHtml(l, r) {
        ...
        var rows = cfg.times.map(function (t) {
            ...
            var hp = isBusy ? null : _hotPct(l, day, tm);   // жёлтая метка скидки на конкретное время
    ...
    /* переключатель режима календаря владельца: занятость дней | точечные скидки */
    ...
    /* панель точечных скидок: свой процент на дату и (опционально) конкретные времена */
    ...
    function _fmrBlocksBuy(l) {
        ...
        if (rr != null && rstat) {
            ...
            rrHtml = '<div class="fmr-line" style="margin-top:5px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">' + rrWarn + 'RR <b style="color:' + rrCol + ';">' + rr + '%</b> <span style="font-size:11px;color:' + rrCol + ';font-weight:600;">' + _esc(rstat) + '</span>' + normTxt + '<i class="fmr-i ti ti-info-circle push" data-fi="rr"></i></div>' +
                '<div class="fmr-info" data-finfo="rr">RR (Reach Rate) = охват ÷ подписчики — какой процент подписчиков видит пост. Норму смотрим по размеру канала (у больших она ниже — это нормально): микро до 5к 25–50%, малый 5–10к 18–35%, средний 10к–100к 7–22%, крупный 100к–1М 6–16%, миллионник 3–10%. Нормы выведены из реальной базы каналов. Слишком низко для своего размера — признак мёртвой базы; в разы выше нормы — повод проверить источник охвата.' + rrAnom + '</div>';
        }
        var erHtml = '';
    ...
    function fullCard(l) {
        ...
        return '<div class="fmx-cwrap"><div class="fmx-card' + (glowOn ? ' fmx-prem' : '') + (fullBg ? ' fmx-fullbg' : '') + '" data-u="' + _esc(l.username) + '">' + cbgHtml + stkHtml + covBdg +
            ...
            (l.er != null ? '<div><div class="l">RR</div><div class="v" style="color:' + hc + ';">' + (Math.round(l.er * 10) / 10) + '%</div></div>' : '') +
            ...
            _ctcLinesHtml(l) +   // реклама/сотрудничество на Площадке (как на Радаре)
    ...
    // Общий рендер контактов (Радар + Площадка): реклама + сотрудничество (чётко помеченные Telegram-ссылки).
    // Ссылка на сам канал сюда не попадает — для перехода есть кнопка «Открыть канал».
    ...
    function _pwMetrics(l) {
        ...
        // RR со статусом и нормой размера + ER с расшифровкой реакций — как на Радаре
        var _xtra = '';
        if (l.er != null && l.reach_status) {
            ...
            _xtra += '<div style="font-size:11px;color:#9aa0b8;margin-top:6px;">RR — <b style="color:' + _rc + ';">' + _esc(l.reach_status) + '</b>' + _nrm + '</div>';
        }
        if (l.engagement_percent != null) {
            var _eb = [];
        ...
        return '<div class="fmx-pwc">' +
            ...
            cell('RR', l.er != null ? (l.er > 100 ? _warnTri(13) + ' ' : '') + String(l.er).replace('.', ',') + '%' : '—', l.er == null) +
            '<div class="pw-mdiv"></div>' +
            ...
            (l.er != null && l.er > 100 ?
                ...
                '<div><b>RR ' + String(l.er).replace('.', ',') + '%</b> — просмотров у постов больше, чем подписчиков в канале. ' +
