(function () {
    ...
    function _audLabel(l) {
        if (l.audience_source === 'commenters' && l.female_pct != null && (l.gender_sample || 0) >= 15) {
            ...
            return { icon: fem ? 'ti-gender-female' : 'ti-gender-male', color: fem ? '#ff6fae' : '#5b9dff',
                ...
                text: '≈' + main + '% комментаторов — ' + (fem ? L('женщины') : 'мужчины'),
    ...
    var FMT_CATALOG = [
        ...
        { k: 'perm',   n: L('Без удаления'), sub: '3 ч в топе · остаётся 30 дней или навсегда · премиум', core: true, preset: 11000 },
        { k: 'native', n: L('Нативный'),     sub: 'Автор пишет сам по ТЗ · доверие и конверсия выше', core: true, preset: 11000 },
        { k: 'repost', n: L('Репост'),       sub: 'Пересылка вашего поста · по правилам 1/24', core: true,        preset: 5000 },
        { k: 'pinned', n: L('Закреп'),       sub: 'Закрепление в шапке · надбавка к формату', core: false,       preset: 2500 },
        { k: 'circle', n: L('Кружок'),       sub: 'Видеосообщение внутри поста',         core: false,            preset: 5000 },
        { k: 'stories', n: L('Сторис'),      sub: 'Отдельный тип площадки, не пост в ленте', core: false,        preset: 3000 }
    ];
    ...
    var COVER_NAMES = [L('Фиолет'), 'Изумруд', 'Закат', 'Океан', 'Огонь', 'Магента'];
    ...
    var TIPS = [
        ...
        [L('Закупщику · читай ER в паре с охватом'), 'Высокий охват при ER около нуля означает, что аудиторию привели, но канал ей неинтересен. Живой канал с ER 5–10% нередко конвертирует лучше крупного с ER 1%. Реакции — лайки, репосты, комментарии — подделать дороже, чем просмотры, поэтому ER точнее показывает вовлечённость.'],
        ...
        [L('Закупщику · релевантность ниши важнее размера'), 'Реклама финансового оффера в профильном канале о финансах отработает лучше, чем во вдвое большем канале о рецептах. Бейдж «В нише» помечает каналы твоей ниши. Узкий, но релевантный канал обычно даёт более дешёвую и качественную конверсию, чем широкий, но нерелевантный охват.'],
        [L('Закупщику · сверяй гео и язык аудитории с оффером'), 'Отличные ER и охват на аудитории из недорогого гео или на чужом языке денег не принесут — оффер платит по конкретным странам. Перед закупкой уточни топ-страны и язык аудитории (у владельца или в статистике канала) и бери канал под гео и язык своего оффера, а не только под нишу.'],
        [L('Закупщику · оценивай сам рекламный пост, не только канал'), 'Даже в сильном канале слабый креатив не даст результата. Продумай пост: понятная выгода в первой строке, один призыв к действию, ссылка с меткой. Один и тот же канал на разных креативах даёт разную цену результата — тестируй заход и формулировку, а не только площадку.'],
        [L('Закупщику · начинай с теста, масштабируй по результату'), 'Первое размещение в новом канале бери минимальным форматом. Замерь по одному посту переходы, подписки и продажи в окне 24–48 часов после выхода (за час рано — пост не добрал, через неделю поздно) и посчитай стоимость целевого действия (потрачено ÷ результат). Сработало — бери пакет и повторяй; нет — потеря невелика, а вывод получен. Не вкладывай весь бюджет в непроверенный канал.'],
        [L('Закупщику · веди список проверенных и чёрный список'), 'Сработавшие каналы записывай и возвращайся к ним пакетами — предсказуемый результат дешевле постоянного поиска. Каналы с завышенной ценой, накруткой или невыполненными договорённостями держи в чёрном списке. Со временем свой проверенный пул важнее любого каталога.'],
        [L('Закупщику · учитывай сезонность цен'), 'CPM не постоянен: в высокий сезон (например, ноябрь–декабрь) спрос на рекламу и цены растут, летом часто проседают. Планируй закуп заранее и сравнивай цену не только между каналами, но и с медианой ниши на текущий момент — то, что весной было дорого, осенью может оказаться в рынке.'],
        [L('Закупщику · учитывай рекламную нагрузку и пересечение аудиторий'), 'Смотри, сколько рекламы в день даёт канал: частые размещения выжигают отклик, и твой пост теряется среди других. При пакетной закупке нескольких каналов одной ниши аудитории пересекаются — рискуешь дважды заплатить за одних и тех же людей. Разноси такие закупки по времени и по разным сегментам.'],
        [L('Закупщику · цена в оффере стартовая — торгуйся'), 'За пакет и повторные размещения уместно просить скидку, для первого теста — сниженную цену или минимальный формат. Опирайся в торге на медиану CPM ниши из Рыночного терминала: если просят заметно выше рынка, это аргумент снизить цену. Фиксированной цены в закупе рекламы обычно нет.'],
        [L('Владельцу · заполняй оффер полностью'), 'Оффер с обложкой, чётким форматом, ценой и живыми метриками собирает больше откликов, чем пустой. Рекламодатель отсеивает каналы за секунды: не видит охват, ER и цену — переходит к следующему. Каждое незаполненное поле — это потерянный клиент.'],
        [L('Владельцу · назначай цену от реального охвата'), 'Считай стоимость размещения через CPM своей ниши. При медиане CPM 300 ₽ и среднем охвате 10 000 ориентировочная цена поста — около 3 000 ₽. Завысишь вдвое — оффер провисит без сделок; занизишь — недозаработаешь. Ориентиры по нишам смотри в Рыночном терминале.'],
        [L('Владельцу · наращивай подтверждённую репутацию'), 'Бейдж «Владелец подтверждён» (бот — администратор канала) и подтверждённые сделки с рейтингом снимают у покупателя главный вопрос «заплачу — и что получу». Канал с тремя сделками и рейтингом 4,8 продаёт дороже и быстрее безымянного. Репутация копится только по сделкам, проведённым через Площадку.'],
        [L('Владельцу · держи метрики честными'), 'Накрутка подписчиков роняет ERR и включает красный светофор — опытный закупщик это видит и уходит. Живой канал с бейджем «Фрод-контроль пройден» вызывает больше доверия, чем формально крупный с выгоревшей базой. Честные метрики — это актив, который работает на цену размещения.'],
        ...
        [L('Обеим сторонам · не забывай о маркировке рекламы'), 'Если реклама направлена на аудиторию страны, где действует обязательная маркировка (в России — токен erid), заполни поле erid в оффере и согласуй маркировку со второй стороной до выхода поста. Ответственность за маркировку несут стороны сделки, а не Площадка.']
    ];
    ...
    function _tSpark(vals, color) {
        ...
        if (vals.length < 3) {
            return '<span class="fmx-t2nospark">' +
                (vals.length ? 'день ' + vals.length + ' из 3' : L('копится')) + '</span>';
        }
    ...
    var _MON_S = [L('янв'), 'фев', 'мар', 'апр', 'мая', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек'];
    ...
    function _tEventHtml(e, isFirst) {
        ...
        var txt = '';
        if (e.type === 'spike') {
            txt = '<b>' + L('Всплеск подписчиков') + '</b> ' + L('в нише') + ' ' + niche + (e.pct ? ': +' + e.pct + '%' + (e.days ? ' за ' + e.days + ' дн' : '') : '') +
            ...
            txt = '<b>' + L('Новый оффер') + '</b> ' + L('в нише') + ' ' + niche + (e.price ? ' — от ' + _num(e.price) + ' ₽' : '');
        } else if (e.type === 'hot') {
            txt = '<b>' + L('Горящая скидка −') + e.pct + '%</b> ' + L('на размещение в нише') + ' ' + niche;
        } else if (e.type === 'suspect') {
            txt = '<b>' + L('Антифрод-метка') + '</b>' + L(': канал в нише') + ' ' + niche + ' — подозрительная кривая просмотров';
            ...
            txt = '<b>' + L('Антифрод за неделю') + '</b>' + L(': помечено') + ' ' + e.count + ' ' + _plural(e.count, 'канал', 'канала', 'каналов') +
                ' — ' + nl + (e.more ? ' и ещё ' + e.more : '');
        } else if (e.type === 'new_channel') {
            txt = '<b>' + L('Новый канал в Радаре') + '</b> ' + L('в нише') + ' ' + niche + ' — ' + _short(e.subs) + ' подписчиков';
        } else if (e.type === 'price_change') {
            txt = '<b>' + L('Оффер изменил цену') + '</b> ' + L('в нише') + ' ' + niche + ': ' + _num(e.old) + ' → ' + _num(e.new) + ' ₽';
        } else if (e.type === 'niche_move') {
            txt = '<b>' + L('CPM в нише') + ' ' + niche + '</b> ' + _tFmtDelta(e.delta) + ' за неделю';
        }
    ...
    function _paintTerminal() {
        ...
        if (_term.generated_at) {
            ...
            if (!isNaN(gm) && gm >= 0) ago = gm <= 0 ? L('обновлено только что') : 'обновлено ' + gm + ' мин назад';
        }
        ...
        var _listHtml = '<div class="fmx-t2list">' + list.map(function (n) {
            ...
            return '<div class="lr" data-tniche="' + _esc(n.niche) + '"><span class="dot" style="background:' + col + ';"></span>' +
                ...
                (n.delta7 != null ? _tFmtDelta(n.delta7, n.thin) : '<span class="fmx-tfl">' + (n.cpm_own === false ? L('оценка') : 'новая') + '</span>') + '</span></div>';
        }).join('') + '</div>';
    ...
    function _modMoneyBody() {
        ...
        return '<div class="fmx-mstatrow" style="border-top:0;margin-top:11px;padding-top:0;"><span>' + L('Оплачено за месяц') + '</span><b>' + _num(_mod.ov.revenue_month_rub) + ' ₽</b></div>' +
            '<div class="fmx-mstatrow"><span>' + L('Расход на ИИ') + '</span><b>$' + sp.toFixed(2) + ' из $' + bud + '</b></div>' +
    ...
    function _modPaintSections() {
        ...
        var h = _modSec('money', 'ti-coin', 'green', 'Деньги', 'выручка · расход на ИИ', money, money ? L('за месяц') : '', _modMoneyBody());
    ...
    function modApprove(id) {
        uiConfirm(L('Одобрить оффер #') + id + ' и опубликовать?', function () {
            apiPost('/api/v1/admin/listing/' + id + '/approve').then(function (r) { _modAfter(r, 'l' + id, 'Одобрено'); }).catch(function () { uiAlert(L('Не удалось. Повтори попытку.')); });
        });
    }
    function modAdultOff(id) {
        uiConfirm(L('Снять метку 18+ с оффера #') + id + '? Если оффер уже одобрен, он сразу выйдет в общую ленту.', function () {
            apiPost('/api/v1/admin/listing/' + id + '/adult', { adult: false }).then(function (r) { _modAfter(r, 'l' + id, 'Метка 18+ снята', true); }).catch(function () { uiAlert(L('Не удалось. Повтори попытку.')); });
        });
    }
    function modReject(id) {
        modPrompt({ title: 'Отклонить оффер #' + id, placeholder: 'Причина (необязательно) — автор увидит и сможет исправить', btn: 'Отклонить', optional: true }, function (reason) {
            apiPost('/api/v1/admin/listing/' + id + '/reject', { reason: reason }).then(function (r) { _modAfter(r, 'l' + id, 'Отклонено'); }).catch(function () { uiAlert(L('Не удалось. Повтори попытку.')); });
        });
    }
    ...
    function modRestore(id) {
        uiConfirm(L('Вернуть заявку R') + id + ' и снять жалобы?', function () {
            apiPost('/api/v1/admin/request/' + id + '/restore').then(function (r) { _modAfter(r, 'r' + id, 'Возвращено'); }).catch(function () { uiAlert(L('Не удалось. Повтори попытку.')); });
        });
    }
    function modRemove(id) {
        modPrompt({ title: 'Снять заявку R' + id, placeholder: 'Причина (необязательно)', btn: 'Снять', optional: true }, function (reason) {
            apiPost('/api/v1/admin/request/' + id + '/remove', { reason: reason }).then(function (r) { _modAfter(r, 'r' + id, 'Снято'); }).catch(function () { uiAlert(L('Не удалось. Повтори попытку.')); });
        });
    }
    ...
    function _collapsible(box, key, total) {
        qsa(box, '[data-more="' + key + '"]').forEach(function (b) {
            b.addEventListener('click', function () {
                ...
                b.textContent = host.classList.contains('open') ? L('Свернуть') : ('Показать всех · ' + total);
            });
        });
    }
    ...
    var _EVN = { app_open: 'Вход в приложение', tariffs: 'Витрина Forge', fn_create_post: L('Создать пост'), fn_rewrite_post: L('Рерайт поста'), fn_content_plan: L('Контент-план'), fn_ai_audit: L('AI-аудит'), fn_ai_strategy: 'ИИ-стратегия', fn_competitor_analysis: L('Анализ конкурентов'), fn_my_channels: L('Мои каналы'), fn_add_channel: L('Подключение канала'), fn_radar: L('Радар'), fn_marketplace: L('Рынок рекламы'), fn_referral: L('Друзья и промокод'), fn_profile: 'Кабинет', fn_find_advertisers: L('Поиск рекламодателей'), fn_post_price: L('Цена поста'), fn_negotiation_templates: L('Шаблоны переговоров'), fn_voice_settings: L('Голос канала'), mx_catalog: 'Радар (раздел)', mx_market: 'Площадка (раздел)', mx_mod: 'Админ-панель', mxs_buy: 'Площадка · Купить', mxs_sell: 'Площадка · Продать', mxs_mine: L('Мои офферы'), mxs_create: 'Конструктор оффера', mxs_deals: 'Сделки' };
    var _OPN = { generate: 'Генерация поста', modify: 'Правка поста', intent: 'Уточняющие вопросы', suggest: 'Подсказки правок', ideas: 'Идеи тем', rewrite: 'Рерайт (ИИ)', voice: 'Голос канала (ИИ)', audit: L('AI-аудит'), strategy: 'ИИ-стратегия', strategy_chat: 'Чат стратегии', competitors: L('Анализ конкурентов'), ad_exchange: 'Биржа (ИИ)', content_plan: 'Контент-план: идея', content_plan_day: 'Контент-план: день', moderation: 'Модерация (платформа)', niche: 'Ниша (платформа)' };
    ...
    function paintCatalogBody() {
        ...
        function tailNote(loading) {
            ...
            if (loading || mounted < list.length || pendingSrv) {
                ...
                t.innerHTML = '<div style="text-align:center;color:#565b73;font-size:11.5px;padding:12px 8px 2px;">' + L('Показано') + ' ' + mounted + (known ? ' из ' + known : '') + ' — дальше подгружается при прокрутке' + (loading ? '…' : '') + '</div>';
                return;
            }
            t.innerHTML = list.length > FIRST ? '<div style="text-align:center;color:#565b73;font-size:11.5px;padding:12px 8px 2px;">' + list.length + ' ' + _plural(list.length, 'канал', 'канала', 'каналов') + ' ' + L('— показаны все, что нашлись по фильтру') + '</div>' : '';
        }
        function mountNext() {
    ...
    function _sellCtaSub() {
        ...
        return L('Оформи оффер — его увидят покупатели');
    }
    ...
    function openBuyFilters() {
        ...
        bg.innerHTML = '<div class="fmx-cfm-box fmx-bf-compact" style="left:50%;transform:translateX(-50%);margin-left:0;width:calc(100vw - 20px);max-width:480px;bottom:12px;"><div class="fmx-cfm-t" style="margin-bottom:10px;display:flex;align-items:center;gap:8px;"><i class="ti ti-adjustments-horizontal" style="color:#818cf8;"></i> ' + L('Фильтры') +
            ...
            '<button class="fmx-fx' + (_sort === 'niche' && _nicheSel ? ' on' : '') + '" data-nf="pick">' + (_sort === 'niche' && _nicheSel ? 'Ниша: ' + _esc(_nicheSel) : L('Выбрать нишу…')) + '</button>' +
            '</div>' +
    ...
    function reqCard(r) {
        ...
        return '<div class="fmx-req' + (r.mine ? ' mine' : '') + '">' +
            '<div class="fmx-reqh">' + head.join('') + '<span class="fmx-reqb' + (r.budget ? '' : ' na') + '">' + (r.budget ? 'до ' + _num(r.budget) + ' ₽' : L('бюджет не указан')) + '</span></div>' +
    ...
    function _mineStrip(l) {
        ...
        function draw(r) {
            ...
            var demand = r.demand || {};
            var WD = [L('Вс'), 'Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб'];
            var MO = [L('янв'), 'фев', 'мар', 'апр', 'май', 'июн', 'июл', 'авг', 'сен', 'окт', 'ноя', 'дек'];
            var h = '';
    ...
    function paintMine() {
        ...
        sub.innerHTML = '<div class="fmx-note"><i class="ti ti-briefcase"></i> ' + L('Кабинет продавца: статус, статистика и управление офферами. Занятые даты отмечай в календаре — покупатели видят свободные.') + '</div>' +
            (lim != null ? '<div style="background:rgba(255,255,255,0.03);border:0.5px solid rgba(255,255,255,0.09);border-radius:14px;padding:12px 13px;margin-bottom:10px;">' +
                ...
                '<span class="num" style="margin-left:auto;font-size:11px;color:#8990a8;">' + L('размещено:') + ' ' + used + ' из ' + lim + '</span></div>' +
            ...
            '<button class="fmx-save" id="fmx-mineNew" style="margin-top:14px;"' + (avail === 0 ? ' disabled style="margin-top:14px;opacity:0.5;"' : '') + '><i class="ti ti-plus"></i> ' + L('Разместить ещё оффер') + (avail != null ? ' · ' + (avail > 0 ? 'доступен ещё ' + avail : L('лимит исчерпан')) : '') + '</button>' +
        ...
        qsa(sub, '[data-mpause]').forEach(function (b) {
            b.addEventListener('click', function () {
                ...
                apiPost('/api/v1/marketplace/listings/' + l.id + '/' + act, {}).then(function (r) {
                    ...
                    l.status_human = act === 'pause' ? 'Заморожен' : L('Опубликовано');
        ...
        qsa(sub, '[data-mdel]').forEach(function (b) {
            b.addEventListener('click', function () {
                ...
                uiConfirm(L('Удалить оффер «') + (l.title || l.username || '') + '» с Площадки навсегда? Оформление и продвижение не сохранятся.', function () {
                    b.disabled = true;
    ...
    function nbProgress() {
        ...
        if (revItems.length) {
            html += group('👁', { bg: 'rgba(245,158,11,0.15)', c: '#fbbf24', br: 'rgba(245,158,11,0.3)' }, 'На ручной проверке', revItems.length, '<span>' + L('Проверим и опубликуем — обычно до суток') + '</span>');
        }
    ...
    var _RF_GEO = [['ru', 'Россия'], ['kz', 'Казахстан'], ['by', 'Беларусь'], ['ua', 'Украина'], ['uz', 'Узбекистан'], ['kg', 'Киргизия'], ['az', 'Азербайджан'], ['ge', 'Грузия'], ['am', 'Армения'], ['tj', 'Таджикистан'], ['tr', 'Турция'], ['ae', 'ОАЭ'], ['sa', 'Саудовская Аравия'], ['eg', 'Египет'], ['ir', 'Иран'], ['in', 'Индия'], ['bd', 'Бангладеш'], ['id', 'Индонезия'], ['vn', 'Вьетнам'], ['us', 'США'], ['gb', 'Великобритания'], ['de', 'Германия'], ['at', 'Австрия'], ['ch', 'Швейцария'], ['fr', 'Франция'], [L('it'), 'Италия'], ['es', 'Испания'], ['pt', 'Португалия'], ['br', 'Бразилия'], ['mx', 'Мексика'], ['ar', 'Аргентина'], ['et', 'Эфиопия'], ['tz', 'Танзания'], ['ke', 'Кения'], ['int', 'Международный']];
    ...
    function openComplaint(target) {
        ...
        el('fmx-rep-send').addEventListener('click', function () {
            ...
            apiPost('/api/v1/marketplace/complaints', body).then(function (r) {
                ...
                toast(r && r.hidden ? 'Скрыто до проверки' : L('Жалоба отправлена — проверим'));
                if (r && r.hidden) {
    ...
    var _SCOPE_LBL = { both: 'Радар + Площадка', radar: L('Радар'), market: L('Площадка') };
    ...
    function renderAlertsList(r) {
        ...
        var h = '<button class="fmx-alnew" id="fmx-al-new"' + (canAdd ? '' : ' disabled') + '><i class="ti ti-plus"></i> ' + L('Новое уведомление') + '</button>' +
            '<div class="fmx-allim">' + L('Активно') + ' ' + used + ' из ' + lim + (canAdd ? '' : ' · достигнут предел') + '</div>';
        if (!alerts.length) {
            ...
            alerts.forEach(function (a) {
                ...
                h += '<div class="fmx-al' + (a.enabled ? '' : ' off') + '">' +
                    ...
                    '<div class="fmx-al-match">' + (a.enabled ? '<span style="color:' + (a.mode === 'digest' ? '#f5bf4f' : '#5DCAA5') + ';">●</span> ' + (a.seen_count ? a.seen_count + ' найдено · ' : '') + (a.mode === 'digest' ? 'сводка раз в день' : 'мгновенно') : '<span style="color:#565b73;">' + L('выключено') + '</span>') + '</div>' +
    ...
    function openAlertEditor(a) {
        ...
        var sh = _alSheet(_alHead((_aEdit.id ? 'Изменить уведомление' : L('Новое уведомление')), 'Условия закупки', true) +
            '<div class="fmx-ae-sec">' + L('Название') + '</div><input class="fmx-inp" id="fmx-ae-name" maxlength="60" value="' + _esc(_aEdit.name) + '">' +
            '<div class="fmx-ae-sec">' + L('Где искать') + '</div><div class="fmx-segw" id="fmx-ae-scope">' + seg(_aEdit.scope, [['both', 'Обе'], ['radar', 'Радар'], ['market', 'Площадка']]) + '</div>' +
            ...
            '<div class="fmx-ae-sec">' + L('Пол аудитории') + '</div><div class="fmx-segw" id="fmx-ae-aud">' + seg((f.aud.male ? 'male' : f.aud.female ? 'female' : ''), [['male', 'Муж'], ['female', 'Жен'], ['', 'Любой']]) + '</div>' +
            ...
            '<div class="fmx-ae-sec">' + L('Как часто уведомлять') + '</div><div class="fmx-segw" id="fmx-ae-mode">' + seg(_aEdit.mode, [['instant', 'Мгновенно'], ['digest', 'Сводка раз в день']]) + '</div>' +
    ...
    function openAlertMatches(id, scope) {
        ...
        function draw() {
            ...
            apiGet('/api/v1/marketplace/alerts/' + id + '/matches?scope=' + cur).then(function (r) {
                ...
                var h = '<div class="fmx-allim" style="margin:0 0 10px;">' + L('Подходит') + ' ' + (r.count || items.length) + ' ' + _plural(r.count || items.length, 'канал', 'канала', 'каналов') + ' ' + L('— нажми, чтобы открыть в Telegram') + '</div>';
    ...
    function calDraw(box, l, mode) {
        ...
        var h = '<div class="fmx-calhead">' +
            ...
            '<div class="fmx-slgrid">' + [L('Пн'), 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map(function (w) { return '<span class="fmx-slw">' + w + '</span>'; }).join('');
        ...
        h += '</div>' +
            ...
            (r.hot ? '<span><i style="background:rgba(245,191,79,0.6);"></i>' + L('горящие') + ' ' + (r.hot.map ? 'до ' : '') + '−' + r.hot.pct + '%</span>' : '') +
            ...
            (r.hot && r.hot.days && r.hot.days.length ? '<div class="fmx-slnote" style="color:#f5bf4f;"><i class="ti ti-discount-2"></i><span>' + L('Точечные скидки:') + ' ' + r.hot.days.length + ' ' + _plural(r.hot.days.length, 'дата', 'даты', 'дат') + ' · до −' + r.hot.pct + '%</span></div>' : '') +
        ...
        if (mode === 'edit') {
            ...
            if (_mo) _mo.addEventListener('click', function () {
                apiPost('/api/v1/marketplace/listings/' + l.id + '/months', { month: mKey, open: !monthOpen }).then(function (rr) {
                    if (!rr || !rr.ok) { _haptic('error'); uiAlert(rr && rr.error === 'forbidden' ? 'Управлять календарём может только владелец оффера' : L('Не удалось изменить месяц')); return; }
                    _haptic('success');
            ...
            b.addEventListener('click', function () {
                ...
                if (mode === 'edit') {
                    if (_ownerCalHot) {
                        ...
                        if (iso > _isoOf(_lim)) { _haptic('error'); uiAlert(L('Скидку можно назначить максимум на ') + (r.horizon_days || 180) + ' дней вперёд'); return; }
    ...
    function _calToggleDay(box, l, iso) {
        apiPost('/api/v1/marketplace/listings/' + l.id + '/slots/toggle', { day: iso }).then(function (rr) {
            if (!rr || !rr.ok) { _haptic('error'); uiAlert(rr && rr.error === 'forbidden' ? 'Управлять датами может только владелец оффера' : L('Не удалось изменить день')); return; }
            _haptic('light');
            var r = _calData[l.id];
            if (rr.busy) { r.busy.push(iso); } else { r.busy = r.busy.filter(function (x) { return x !== iso; }); }
            r.slots_updated_at = new Date().toISOString();
    ...
    function _buyerSlotsHtml(l, r) {
        ...
        var rows = cfg.times.map(function (t) {
            ...
            return '<div class="fmx-tsl' + (isBusy ? ' bs' : (sel ? ' sel' : '')) + (pk ? ' peak' : '') + (hp ? ' hot' : '') + '"' + (isBusy ? '' : ' data-ts="' + tm + '"') + '>' +
                ...
                '<span class="st">' + (isBusy ? 'Занято' : (sel ? 'Выбрано ✓' : L('Свободно'))) + '</span></div>';
        }).join('');
        ...
        apiPost('/api/v1/marketplace/listings/' + l.id + '/hot-day', {
            ...
            if (!rr || !rr.ok) {
                ...
                var m = { forbidden: 'Управлять скидками может только владелец оффера',
                    out_of_horizon: L('Скидку можно назначить максимум на ') + ((_calData[l.id] || {}).horizon_days || 180) + ' дней вперёд',
            ...
            if (_ownerSelDay) {
                ...
                var rows = cfg.times.map(function (t) {
                    ...
                    return '<div class="fmx-tsl' + (pk ? ' peak' : '') + '" data-otog="' + tm + '"><span class="tm">' + t.t + '</span>' +
                        (pk ? '<span class="fmx-peaktag">' + L('час пик') + '</span>' : '') +
                        '<span class="st">' + (isBusy ? 'Занято' : L('Свободно')) + '</span>' +
    ...
    function drawBuyerSlots(box, l) {
        ...
        h += '</div>' +
            ...
            (r.hot ? '<span><i style="display:inline-block;width:8px;height:8px;border-radius:3px;background:rgba(245,191,79,0.6);"></i> ' + L('горящие') + ' ' + (r.hot.map ? 'до ' : '') + '−' + r.hot.pct + '%</span>' : '') +
    ...
    function openWriteSheet(l) {
        ...
        sh.innerHTML = '<div class="grip"></div><h3>' + L('Сообщение владельцу канала') + '</h3>' +
            ...
            '<span>' + L('Дата:') + ' <b id="fmx-wd">' + ((f.basket && f.basket.length) ? (f.basket.length + (f.day ? 1 : 0)) + ' ' + _plural(f.basket.length + (f.day ? 1 : 0), 'дата', 'даты', 'дат') : (f.day ? _fmtDayRu(f.day) + (f.time != null ? ' ' + _fmtT(f.time) : '') : 'уточню в чате')) + '</b></span>' +
    ...
    function _tedUpload(kind) {
        ...
        inp.onchange = function () {
            ...
            if (!f) return;
            if (f.size > MEDIA_MAX_BYTES) { uiAlert(L('Файл ') + Math.round(f.size / 1048576) + ' МБ — больше лимита. Выбери файл меньше.'); return; }
            var fd = new FormData();
    ...
    function toggleCmp(u) {
        ...
        if (_cmp[u]) { delete _cmp[u]; }
            ...
            if (_cmpCount() >= CMP_MAX) { _haptic('error'); toast(L('Можно сравнить не больше ') + CMP_MAX + ' каналов — сними лишний.'); return; }
    ...
    function drawCmpBar() {
        ...
        if (!n) return;
        bar.innerHTML = '<span class="fmx-cmpn">' + L('Выбрано') + ' ' + n + ' из ' + CMP_MAX + '</span>' +
    ...
    function updateAccSummaries() {
        var m = {
            ...
            fx: (function () { var n = (_ss.glass !== 'none' ? 1 : 0); if (_ss.glowCard) n++; if (_ss.btns === 'accent') n++; return n ? n + ' актив.' : L('Выключены'); })(),
            ...
            price: (function () { var on = _sfmts.filter(function (f) { return f.on; }); if (!on.length) return L('Не выбраны'); return on.length + ' форм. · от ' + _num(Math.min.apply(null, on.map(function (f) { return f.p; }))) + ' ₽'; })(),
    ...
    function panePrice() {
        ...
        var ins = '<span class="fmx-lbl fmx-mt2"><i class="ti ti-chart-line"></i> ' + L('Аналитика в витрине') + '</span>' +
            '<div class="fmx-htog" id="fmx-hideIns"><div class="fmx-htl">' + L('Показывать час пик и рекл. охват') + '<i>' + (_ss._hideInsights ? 'Скрыто · видно только тебе' : 'Считает площадка по метрикам канала') + '</i></div><div class="fmx-hsw' + (_ss._hideInsights ? '' : ' on') + '"></div></div>';
    ...
    function openPosterStudio() {
        ...
        function uploadPosterBg(file) {
            if (file && file.size > MEDIA_MAX_BYTES) {
                return Promise.reject(new Error(L('Файл ') + Math.round(file.size / 1048576) + ' МБ — это больше 64 МБ'));
            }
        ...
        function waitSend(sec) {
            ...
            (function tick() {
                ...
                b.innerHTML = '<i class="ti ti-clock"></i> ' + L('Можно через') + ' ' + sec + ' с';
                sec--;
        ...
        el('fmx-ps-send').addEventListener('click', function () {
            ...
            function proceed() {
                ...
                if (bgErr) {
                    ...
                    toast(big ? L('Фон слишком большой: до 64 МБ. Для видео нужен короткий ролик — в постер идёт первый отрезок 20 сек') : ('Фон не загрузился: ' + bgErr), true);
                    return;
                }
        ...
        ['cover', 'avatar', 'cardbg'].forEach(function (t) {
            chain = chain.then(function () {
                ...
                if (m.file.size > MEDIA_MAX_BYTES) {
                    throw new Error(L('Файл «') + t + '» ' + Math.round(m.file.size / 1048576) + ' МБ — больше 64 МБ. Выбери файл меньше');
                }
                var fd = new FormData();
    ...
    function _actBadge(l) {
        ...
        var MON_RD = [L('января'), 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября', 'ноября', 'декабря'];
    ...
    function _fmrBlocksBuy(l) {
        ...
        if (!dead && l.ad_er_posts != null) {
            ...
            if (_adv != null) {
                ...
                else if (_adr != null && _adr > 0) { _adCol = '#f87171'; _adTag = 'в ' + String(Math.round(10 / _adr) / 10).replace('.', ',') + ' раза ниже обычного'; }
                else { _adCol = '#818cf8'; }
                _adSub = ' <span style="font-size:11px;color:#565b73;">' + L('— по') + ' ' + _adn + ' ' + _plural(_adn, 'рекламному посту', 'рекламным постам', 'рекламным постам') + '</span>';
            } else if (_adn > 0) {
    ...
    function _heroTiles(l, mode) {
        ...
        if (mode === 'market') {
            ...
            priceSub = (l.owner_price ? L('цена владельца') : (l.price_negotiable ? 'договорная' : (l.price_floored ? 'минимум ниши' : L('оценка ниши'))));
        }
        var dead = _deadT;
        ...
        var rows =
            ...
            _kmRow(L('Охват'),
                _kmSub(dead ? L('не публикует') : ((typeof l.ad_reach_24h === 'number' && l.ad_reach_24h > 0) ? L('замер рекламных постов') : 'медиана постов'), dead ? '#ef4444' : ''),
                dead ? '—' : (av ? '~' + _exact(av) : '—'), '#e8e8ed') +
            _kmRow(L('ERR'),
                dead ? _kmSub(L('нет свежих постов'), '') : _kmPill(rstat || 'уточняется', rstat ? rrCol : '#8d93a8'),
            ...
            _kmRow('CPM, ₽',
                _kmSub(dead ? L('нет охвата') : (l.price_floored ? 'охват мал' : (isOwner ? 'от цены владельца' : L('ориентир ниши'))), ''),
    ...
    function simpleCard(l) {
        ...
        if (pp) {
            ...
            ad = '<div class="fmr-sec num"><span class="kn">1</span>' + L('Стоимость размещения') + ' <span style="font-size:10px;color:#565b73;text-transform:none;letter-spacing:0;font-weight:600;">' + (est ? '· оценка ниши' : '· цена владельца') + '</span> <span class="fmr-i push" data-fi="ad">?</span></div>' +
                '<div class="fmr-line">' + L('Пост') + ' <b class="fmr-big">' + (l.price_negotiable ? 'от ≈' + _num(pp) + ' ₽ · договорная' : (ph ? '≈' + _num(pp) + '–' + _num(ph) + ' ₽' : (est ? 'от ≈' + _num(pp) + ' ₽' : 'от ' + _num(pp) + ' ₽'))) + '</b>' + priceTag + '</div>' +
        ...
        if (pp && av && !dead) {
            ...
            flow = '<div class="fmr-sec num"><span class="kn">3</span>' + L('Перелив · набрать подписчиков') + ' <span class="fmr-i push" data-fi="flow">?</span></div>' +
                cpfHead +
                '<div class="fmr-line" data-flow="1" data-pp="' + pp + '" data-av="' + av + '">Своя конверсия <input class="fmr-conv" type="number" min="0.1" max="100" step="0.5" value="' + conv + '"> % → <b class="fmr-cps" style="color:#5DCAA5;">≈' + _num(cps) + ' ₽</b> <span style="font-size:11px;color:#565b73;">' + L('CPF · цена подписчика') + '</span></div>' +
                '<div class="fmr-sub"><span class="fmr-gained">' + _gainTxt(_grw) + '</span> ' + L('за') + ' <b>≈' + _num(pp) + ' ₽</b> (' + (est ? 'нижняя граница цены' : 'минимальная цена') + ')</div>' +
                '<div class="fmr-warn">' + L('Ниже 0.3% — стоимость подписчика непропорционально высока. Для холодного трафика норма 0.3–1.5%, для прогретой аудитории — выше.') + '</div>' +
                '<div class="fmr-info" data-finfo="flow">' + L('Конверсия — какая доля увидевших пост подпишется именно к тебе. Не знаешь свою — оставь 0,5%: это медиана рынка для первых закупов. Её задают прогрев аудитории, прелендинг (прокладка) и ниша: холодный трафик — единицы процентов, прогретая тёплая аудитория — десятки. После размещений со ссылкой отслеживания сюда подставляется фактическая конверсия канала по замерам сделок. Прогноз, не гарантия: точную цену подписчика видно только по итогам размещения.') + '</div>';
        }
        var struct = '';
    ...
    function _audBlock(l) {
        ...
        if (l.channel_created_ts) {
            ...
            var age = yrs > 0 ? (yrs + ' г' + (rm ? ' ' + rm + ' мес' : '')) : (months >= 1 ? (months + ' мес') : L('меньше месяца'));
    ...
    function _spikeLine(l) {
        ...
        var when = sp.days <= 1.5 ? L('за сутки') : ('за ' + Math.round(sp.days) + ' дн');
    ...
    function openBadgeGuide() {
        ...
        var body =
            healthCard +
            card('<span class="fmx-bdg fmx-b-owner"><i class="ti ti-user-check"></i>' + L('Владелец') + '</span>', 'Владелец подтверждён',
            ...
            card('<span class="fmx-bdg fmx-b-nofraud"><i class="ti ti-shield-check"></i>' + L('Фрод-контроль пройден') + '</span>', 'Фрод-контроль пройден',
            ...
            card('<span class="fmx-bdg" style="color:#c7ccf7;border:0.5px solid rgba(129,140,248,0.35);background:rgba(129,140,248,0.12);"><span class="fmx-rknic">' + RKN_SVG + '</span>' + L('Зарегистрирован в РКН') + '</span>', 'Зарегистрирован в РКН',
            ...
            card('<span class="fmx-bdg fmx-b-safe"><i class="ti ti-shield-check"></i>' + L('Безопасный') + '</span>', 'Безопасный',
            ...
            card('<span class="fmx-bdg fmx-b-dead"><i class="ti ti-alert-triangle"></i>' + L('Требует проверки') + '</span>', 'Требует проверки',
            ...
            '</div>' +
            card('<span class="fmx-bdg fmx-b-big"><i class="ti ti-crown"></i>' + L('Крупный') + '</span>', 'Крупный',
                'В канале от 100 000 подписчиков. Большой охват за размещение — подходит для масштабных запусков и широких проливов.') +
            card('<span class="fmx-bdg fmx-b-match"><i class="ti ti-target-arrow"></i>' + L('В нише') + '</span>', 'В нише',
            ...
            card('<span class="fmx-bdg fmx-b-deal"><i class="ti ti-heart-handshake"></i>' + L('★ 4,8 · 3 сделки') + '</span>', 'Сделки и рейтинг',
            ...
            card('<span class="fmx-bdg" style="color:#f5bf4f;border-color:rgba(245,191,79,0.45);background:rgba(245,191,79,0.1);"><i class="ti ti-discount-2"></i>' + L('Горящие даты до −15%') + '</span>', 'Горящие даты',
            ...
            card('<span class="fmx-aud" style="color:#5b9dff;border:0.5px solid #5b9dff55;background:#5b9dff1a;border-radius:99px;padding:3px 9px;font-size:10px;font-weight:700;"><i class="ti ti-gender-male"></i> ' + L('Мужская') + '</span>', 'Аудитория: мужская',
            ...
            card('<span class="fmx-aud" style="color:#ff6fae;border:0.5px solid #ff6fae55;background:#ff6fae1a;border-radius:99px;padding:3px 9px;font-size:10px;font-weight:700;"><i class="ti ti-gender-female"></i> ' + L('Женская') + '</span>', 'Аудитория: женская',
                'Больше половины читателей — женщины. Подходит под офферы с женской целевой аудиторией: красота, мода, дети, дом, маркетплейсы.') +
            card('<span class="fmx-aud" style="color:#9aa0b5;border:0.5px solid #9aa0b555;background:#9aa0b51a;border-radius:99px;padding:3px 9px;font-size:10px;font-weight:700;"><i class="ti ti-users-group"></i> ' + L('Смешанная') + '</span>', 'Аудитория: смешанная',
    ...
    function _gainTxt(g) {
        if (!(g > 0)) return L('подписчиков не будет');
        if (g < 1) return L('меньше одного подписчика');
        var v = Math.round(g);
    ...
    function _growPill(l) {
        ...
        return _chmPill((g > 0 ? '+' : '−') + _num(Math.abs(g)) + ' за 30 дн', g > 0 ? '#5DCAA5' : '#f06978');
    }
    function _errPill(l) {
    ...
    function _errNote(l) {
        if (l.er == null) return L('нет свежих постов');
        return (l.reach_norm && l.reach_norm.length === 2)
            ? 'охват к подписчикам · норма для ' + _esc(l.reach_tier || '') + ' ' + l.reach_norm[0] + '–' + l.reach_norm[1] + '%'
            : L('охват к подписчикам');
    }
    function _erPill(l) {
        var v = l.engagement_percent;
        if (v == null) return _chmPill((l.react_count || l.comment_count) ? 'сигнала мало' : L('не измеряется'), '#8990a8');
        return _chmPill(v >= 3.5 ? L('высокая') : (v >= 1 ? L('норма') : L('низкая')),
                        v >= 3.5 ? '#5DCAA5' : (v >= 1 ? '#818cf8' : '#E8B04B'));
    }
    function _erNote(l) {
        var b = [];
        if (l.react_count) b.push('~' + _num(l.react_count) + ' ' + _plural(l.react_count, 'реакция', 'реакции', 'реакций'));
        if (l.comment_count) b.push(_num(l.comment_count) + ' ' + _plural(l.comment_count, 'комментарий', 'комментария', 'комментариев'));
        return b.length ? 'вовлечённость · по ' + b.join(', ') + ' на пост' : 'вовлечённость на пост';
    }
    function _idxPill(l) {
        var c = l.health_class === 'green' ? '#5DCAA5' : (l.health_class === 'red' ? '#f06978' : '#E8B04B');
        var t = l.health_class === 'green' ? 'высокий' : (l.health_class === 'red' ? 'низкий' : 'средний');
        return _chmPill(t, c);
    }
    function _nicheBand(l) {
        ...
        if (d == null || Math.abs(d) <= 15) tail = ' · в рынке';
        else tail = d > 0 ? ' · выше рынка на ' + Math.round(d) + '%' : ' · ниже рынка на ' + Math.abs(Math.round(d)) + '%';
        return 'ниша ' + _num(lo) + '–' + _num(hi) + ' ₽' + tail;
    }
    function _audBar(l) {
        var a = _audLabel(l);
        ...
        return '<div class="fmx-chmaud"><div class="fmx-chmauh"><span class="fmx-chml">' + L('Аудитория') + '</span>' +
            '<b>' + _esc(a.short.replace('муж', 'мужчины').replace('жен', 'женщины')) + '</b></div>' +
    ...
    function _pwMetrics(l) {
        ...
        return '<div class="fmx-pwc">' +
            ...
            '<div class="fmx-chmn">' + (dead ? 'канал не публикует' : (ad ? L('замер рекламных постов') : 'медиана за 30 дней')) + '</div></div>' +
            '<div class="fmx-chmbig"><div class="fmx-chml">CPM</div>' +
            '<div class="fmx-chmv num">' + (cpm != null ? _num(cpm) + ' ₽' : '—') + '</div>' +
            '<div class="fmx-chmn">' + (cpm == null ? (dead ? L('нет охвата') : 'цена не указана') : (_nicheBand(l) || L('за 1000 просмотров'))) + '</div></div>' +
            '</div>' +
            '<div class="pw-spark" id="fmx-pwspark"></div>' +
            '<div class="fmx-chmrows">' +
            _chmRow(L('Подписчики'), (l.subscribers != null ? _num(l.subscribers) : '—') +
                    (_chAge(l.channel_created_ts) ? ' · на рынке ' + _chAge(l.channel_created_ts) : ''),
                    _growPill(l)) +
            ...
            (dead ? '' : _chmRow('ER' + (l.engagement_percent != null ? ' ' + String(l.engagement_percent).replace('.', ',') + '%' : ''),
                    _erNote(l), _erPill(l))) +
            (l.health_score != null ? _chmRow('Индекс канала ' + l.health_score, 'сводная оценка качества', _idxPill(l)) : '') +
            (ad ? _chmRow('Рекламный охват', 'замер за 24 часа после выхода', '<b class="num" style="color:#cdd0de;">~' + _num(ad) + '</b>') : '') +
            '</div>' +
            _audBar(l) +
    ...
    function _trustRows(l) {
        ...
        rows.push({ i: 'ti-heart-handshake', c: '#f5bf4f',
            t: dealN ? L('Подтверждённых сделок:') + ' <b>' + dealN + '</b>' + (l.ad_reach_24h ? ' · охват замерен' : '')
    ...
    function openListing(u, known) {
        ...
        if (l.formats && l.formats.length) {
            ...
            fmtsHtml = '<div class="fmx-lssect">' + L('Форматы и цены') + '</div>' +
                ...
                '<div style="font-size:10px;color:#565b73;margin-top:5px;">' + L('Зелёная точка — лучший CPM.') + ' ' + (l.ad_reach_24h ? L('CPM по') + ' <b style="color:#5DCAA5;">' + L('охвату рекламного поста за первые 24 часа (ERR24)') + '</b> ' + L('— по фактическим замерам сделок.') : 'Тап по строке подставит формат в сообщение') + '</div>' +
                _termsBlock(l);
        }
    ...
    function _apkLimitLine() {
        ...
        return 'Недостаточно средств: нужно ' + fa(price, 11) + L(', на балансе ') + fa(balance, 11);
    }
    ...
    function _apkRenderIntro() {
        ...
        b.innerHTML =
            ...
            '<div style="font-size:12px;color:#8d93a8;line-height:1.5;">' + L('Куда закупать рекламу') + (chName ? ' ' + L('каналу') + ' <b style="color:#c2c6d2;">' + _esc(chName) + '</b>' : ' твоему каналу') + L('. Система отбирает площадки, где реклама может сработать:') + '</div>' +
    ...
    function _apkRenderResult(run) {
        ...
        var n = picks.length;
        b.innerHTML =
            '<div style="font-size:12px;color:#8d93a8;line-height:1.5;margin-bottom:4px;">' + L('Готово:') + ' <b style="color:#e8e8ed;">' + n + ' ' + _plural(n, 'канал', 'канала', 'каналов') + '</b> ' + L('под твою аудиторию') + (run.created_at ? ' · ' + _apkDate(run.created_at) : '') + L('. Отранжированы по шансу, что реклама сработает.') + '</div>' +
            cards +
