(function () {
    ...
    var TERMS = [
        ...
        [L('ROI · окупаемость'), L('Чистая прибыль ÷ затраты на размещение × 100%. Пример: пост за 5 000 ₽ принёс продаж на 12 000 ₽ при себестоимости 4 000 ₽ — прибыль 3 000 ₽, ROI 60%. Выше нуля — размещение в плюсе, ниже — в минусе. AI-разбор оффера даёт в прогнозе плитку с выводом по ROI — оценку, окупится ли размещение при текущей цене. Фактический ROI считай по своим продажам через метку в ссылке.')],
    ...
    function fullCard(l) {
        ...
        return '<div class="fmx-cwrap' + (glowOn ? ' fmx-goldw' : '') + '"><div class="fmx-card' + (glowOn ? ' fmx-prem' : '') + (fullBg ? ' fmx-fullbg' : '') + (noHead ? ' fmx-nohead' : '') + '" data-u="' + _esc(l.username) + '">' + cbgHtml + stkHtml +
            ...
            (l.health_score != null ? '<div class="fmr-info" data-finfo="health">' + L('Индекс здоровья канала (0–100): насколько канал живой и качественный как площадка — вовлечённость, ERR, стабильность охватов, нет ли накрутки. Считается из тех же метрик, что видны выше, поэтому не противоречит им. Зелёный — хорошо, жёлтый — средне, красный — с осторожностью.') + '</div>' : '') +
            alertsHtml +
    ...
    function _apkRenderProgress(phase) {
        ...
        var s = phase === 'analyzing' ? L('Кандидаты найдены — ранжирую и готовлю обоснования. Запуск списан.') : L('Ниша, выгорание ленты, охват, антифрод — без ИИ, запуск ещё не списан');
