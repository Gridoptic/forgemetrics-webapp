(function () {
    ...
    function renderShowcase() {
        ...
        var rows = [
            ...
            [SG('Трафик под твой регион.'), SG('Площадки ранжируются по отдаче в твоей стране: где публикация без ограничений, где через VPN или обходы — с пометкой, но без запретов. Платный перелив с расчётом «бюджет → подписчики» по CPM ниши, доноры из Радара, ссылка отслеживания на каждый источник: видно, что дало приток, а что нет')],
            [SG('Все модели заработка.'), SG('Реклама в канале, перелив трафика, партнёрки, свой продукт. По каждой: с какого размера канала включать, сколько это даёт в месяц в твоей нише и что подготовить заранее')],
