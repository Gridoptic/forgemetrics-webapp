(function () {
    ...
    function forge(n, size) {
        return (typeof window.forgeAmount === 'function') ? window.forgeAmount(n, size || 13) : num(n);
    }
    ...
    function renderShowcase() {
        ...
        var rows = [
            [SG('Ниша и точка старта.'), SG('Есть канал — стратег оценит его по реальным данным и скажет, что усилить. Нет канала или он пустой — разберёт выбранную тобой тему или подберёт нишу под твой опыт и покажет, чем выделиться.')],
        ...
        var price = locked
            ? '<div class="stg-fprice"><b>' + forge(nprice, 25) + '</b><span>' + esc(T('разово')) + '</span></div>' +
